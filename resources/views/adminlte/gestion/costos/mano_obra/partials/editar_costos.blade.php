<table class="table-responsive table-bordered table-striped" style="width: 100%; border: 1px solid #9d9d9d" id="edit_table">
    ...
    <tr id="tr_edit_1">
        <td class="text-center" style="border-color: #9d9d9d">
            <select id="edit_actividad_1" style="width: 100%" onchange="buscar_moByActividad(1)">
        ...
        <td class="text-center" style="border-color: #9d9d9d" id="edit_td_mo_1">
        </td>
...
<script>
    ...
    function add_edit_form() {
        ...
        $('#edit_table').append('<tr id="tr_edit_' + num_form + '">' +
            '<td class="text-center" style="border-color: #9d9d9d">' +
            '<select id="edit_actividad_' + num_form + '" style="width: 100%" onchange="buscar_moByActividad(' + num_form + ')">' +
            $('#edit_actividad_1').html() +
            '</select>' +
            '</td>' +
            '<td class="text-center" style="border-color: #9d9d9d" id="edit_td_mo_' + num_form + '">' +
            '</td>' +
    ...
    function buscar_moByActividad(form) {
        datos = {
            actividad: $('#edit_actividad_' + form).val(),
            form: form
        };
        get_jquery('{{url('gestion_mano_obra/buscar_moByActividad')}}', datos, function (retorno) {
    ...
    function save_costo(form) {
        ...
        $('#tr_edit_' + form).LoadingOverlay('show');
        $.post('{{url('gestion_mano_obra/save_costo')}}', datos, function (retorno) {
            $('#edit_btn_' + form).hide();
            if (retorno.success) {
