@for($i=0;$i<$cantForms;$i++)
        ...
        <div class="well sombra_estandar">
            ...
            <legend style="font-size: 1.3em">
                ...
                <div>
                    ...
                    <input type="date" id="fecha_envio_{{$i+1}}" class="form-control" value="{{$fechaEnvio[$i]->fecha_envio}}" style="width: 25%;margin-bottom: 5px">
                </div>
                <a href="javascript:void(0)" class="btn btn-xs btn-warning pull-right" title="Reiniciar envío" onclick="delete_input('{{$i+1}}')"" id="btn_reset_form_envio_">
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-control" id="cantidad_{{$i+1}}_{{$rows}}" name="cantidad_{{$i+1}}_{{$rows}}" required>
                                        ...
                                        @for($j=0; $j<$cantidad; $j++)
                                            <option @php  if($detalleEnvio->cantidad == $cantidad) echo "selected='selected'"@endphp   value="{{$j+1}}"> {{$j+1}} </option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
    ...
    <script>add_form_envio('{{$i+1}}','{{$dataDetallesPedidos[$i]->cantidad}}')</script>
@endfor
