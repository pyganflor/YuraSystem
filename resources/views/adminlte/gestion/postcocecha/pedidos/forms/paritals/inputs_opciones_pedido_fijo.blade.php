    ...
    <tbody>
        <tr>
            ...
            <td>
                ...
                <input disabled type="date" id="fecha_desde_pedido_fijo" onchange="verificar_intervalo_fecha()" name="fecha_desde_pedido_fijo" class="form-control" required>
            </td>
            <td>
                <label for="Especificaciones">Hasta</label><br />
                <input disabled type="date" id="fecha_hasta_pedido_fijo" onchange="verificar_intervalo_fecha()" name="fecha_hasta_pedido_fijo" class="form-control" required>
            </td>
            @if($opcion != 2 && $opcion != 3)
            <td>
...
@else
    <table width="100%" class="table table-responsive table-bordered" style="font-size: 0.8em; border-color: #9d9d9d"  id="table_content_fechas_pedidos_personalizados">
        <thead>
        <tr style="background-color: #dd4b39; color: white">
            <th class="text-center table-{{getUsuario(Session::get('id_usuario'))->configuracion->skin}}"
                style="border-color: #9d9d9d">
            ...
            <th class="text-center table-{{getUsuario(Session::get('id_usuario'))->configuracion->skin}}"
                style="border-color: #9d9d9d">
                <button type="button" onclick="add_fechas_pedido_fijo_personalizado()" title="Agregar fecha" class="btn btn-success btn-xs" id="btn_add_fechas_pedido_fijo_personalizado"> <i class="fa fa-plus" aria-hidden="true"></i> </button>
                <button type="button" onclick="delete_fechas_pedido_fijo_personalizado()" title="Eliminar fecha" id="btn_delete_fechas_pedido_fijo_personalizado"  class="btn btn-danger btn-xs hide"> <i class="fa fa-trash " aria-hidden="true"></i> </button>
            </th>
        </tr>
        </thead>
        <tbody>
