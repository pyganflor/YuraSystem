<div style="overflow-x: scroll">
    <table class="table-bordered table-responsive" width="100%" style="border: 2px solid #9d9d9d" id="table_semanal_postcosecha">
        @php
            $totales_mes = [];
        @endphp
        <thead>
        <tr>
            ...
            @foreach($data['labels'] as $pos => $label)
                <th class="text-center" style="border-color: white; background-color: #357ca5; color: white"
                    colspan="{{$acumulado == 'false' ? count($data['semanas']) + 1 : count($data['semanas'])}}">
                    {{$label}}
                </th>
            @endforeach
            @if($acumulado == 'false')
...
<script>
    estructura_tabla('table_semanal_postcosecha', false);
</script>