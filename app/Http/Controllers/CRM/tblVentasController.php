...
class tblVentasController extends Controller
{
    ...
    public function filtrar_tablas(Request $request)
    {
        ...
        if ($request->desde != '' && $request->hasta != '') {
            if ($request->rango == 'A') {   // Anual
                ...
                $view = 'mensual';
                if ($request->desde >= 1 && $request->desde <= 12 && $request->hasta >= 1 && $request->hasta <= 12 && $request->desde <= $request->hasta) {
                    $data = $this->getTablasByRangoMensual($request->desde, $request->hasta, $request->cliente, $request->variedad, $annos, $request->criterio);
                } else {
                    return '<div class="alert alert-warning text-center">Los meses ingresados están incorrectos</div>';
                }
            }
    ...
    public function navegar_tabla(Request $request)
    {
        if ($request->desde >= 1 && $request->desde <= 12 && $request->hasta >= 1 && $request->hasta <= 12 && $request->desde <= $request->hasta) {
            $view = 'mensual';
            $data = $this->getTablasByRangoMensual($request->desde, $request->hasta, $request->filtro_cliente, $request->filtro_variedad, [$request->anno], $request->criterio);
        } else {
            return '<div class="alert alert-warning text-center">Los meses ingresados están incorrectos</div>';
        }
    ...
    public function getTablasByRangoMensual($mes_inicial, $mes_final, $cliente, $variedad, $annos, $criterio)
    {
        sort($annos);
        $labels = $annos;
        $meses = [];
