                ...
                foreach ($ciclos as $c) {
                    ...
                    if ($num_sem >= 6 && $num_sem <= 11 && $c->fecha_cosecha == ''/* && $acumulado_c >= $min_temp*/) {
                        if ($acumulado_c >= $min_temp && $acumulado_c < $temp_prom) {   // rango de temperaturas minimas
                            dump($c->modulo->nombre . ' - ' . $num_sem . ', acum: ' . $acumulado_c . ', rango de temperaturas minimas');
                        } else if ($acumulado_c > $temp_prom && $acumulado_c <= $max_temp) {    // rango de temperaturas maximas
                            dump($c->modulo->nombre . ' - ' . $num_sem . ', acum: ' . $acumulado_c . ', rango de temperaturas maximas');
                        } else {    // temperatura promedio
                            dump($c->modulo->nombre . ' - ' . $num_sem . ', acum: ' . $acumulado_c . ', rango de temperatura promedio');
                        }
                    }
                }
            }
